...
static Ref<Environment> _lx_get_env(Node* self) {
    ...
    WorldEnvironment* we = _lx_find_we((Node*)self->get_tree()->get_root()); // <-- AÑADIDO (Node*)
    if (!we) return {};
...
static WorldEnvironment* _lx_find_we_node(Node* self) {
    ...
    return _lx_find_we((Node*)self->get_tree()->get_root()); // <-- AÑADIDO (Node*)
}
...
// ════════════════════════════════════════════════════════════════════
//  LightingSkyNode — 6 sky presets with custom shaders
//  PRESET_PROCEDURAL : Godot ProceduralSkyMaterial (fast, simple)
//  PRESET_ATMOSPHERIC: Rayleigh + Mie scattering, stars, sun disk
//  PRESET_CARTOON    : Cel-shaded flat bands
//  PRESET_ANIME      : Soft pastel gradient with banding
//  PRESET_NIGHT      : Deep starfield + Milky Way + moon
//  PRESET_SUNSET     : Warm tricolor gradient with sun glow
// ════════════════════════════════════════════════════════════════════
...
uniform vec3 sky_top_color : source_color = vec3(0.10,0.20,0.60);
uniform vec3 sky_horizon_color : source_color = vec3(0.50,0.70,0.90);
uniform vec3 ground_color : source_color = vec3(0.10,0.08,0.06);
uniform float rayleigh_strength : hint_range(0.0,5.0) = 1.0;
uniform vec3 rayleigh_color : source_color = vec3(0.30,0.50,1.00);
...
uniform vec3 horizon_fog_color : source_color = vec3(0.70,0.80,0.90);
uniform float horizon_fog_amount : hint_range(0.0,1.0) = 0.30;
uniform float horizon_sharpness : hint_range(0.5,20.0) = 4.0;
uniform vec3 sunset_color : source_color = vec3(1.00,0.40,0.10);
...
float mie_phase(float c,float g){ float g2=g*g; return (1.0-g2)/(4.0*PI*pow(max(1.0+g2-2.0*g*c,0.0001),1.5)); }
...
void sky(){
    ...
    vec3 col=mix(sky_top_color,sky_horizon_color,clamp(sky_t,0.0,1.0));
    float day=clamp(sun_elev*3.0+0.6,0.0,1.0);
    col+=rayleigh_color*rayleigh_phase(cos_t)*rayleigh_strength*day*0.4;
    col+=mie_color*mie_phase(cos_t,mie_eccentricity)*mie_strength*day*0.2;
    float horiz=pow(max(0.0,1.0-alt*horizon_sharpness),5.0);
    col=mix(col,horizon_fog_color,horiz*horizon_fog_amount);
    float ss_h=clamp((0.3-abs(sun_elev))*4.0,0.0,1.0);
    float ss_a=pow(max(0.0,cos_t+0.3),3.0)*ss_h;
    float horiz_low=pow(max(0.0,1.0-alt*6.0),4.0);
    col=mix(col,sunset_color,ss_a*horiz_low*sunset_strength*0.5);
    float night=clamp(-sun_elev*3.0,0.0,1.0);
    col=mix(col,sky_top_color*0.05,night*0.9);
    if(star_density>0.0&&star_brightness>0.0){
        float sv=clamp(0.5-sun_elev*4.0,0.0,1.0);
    ...
    if(LIGHT0_ENABLED){
        ...
        col+=sun_disk_color*LIGHT0_COLOR*LIGHT0_ENERGY*corona*sun_energy*0.4;
        if(sa<sr){
            float tt=sa/max(sr,0.0001);
            float limb=mix(0.5,1.0,sqrt(max(0.0,1.0-tt*tt)));
            col+=sun_disk_color*LIGHT0_COLOR*LIGHT0_ENERGY*limb*sun_energy*6.0;
        }
    }
    col*=sky_exposure;
...
uniform vec3 ground_color : source_color = vec3(0.30,0.55,0.20);
...
float noise2c(vec2 p){ vec2 i=floor(p); vec2 f=fract(p); f=f*f*(3.0-2.0*f);
...
void sky(){
    ...
    vec3 col=mix(sky_horizon_color,sky_top_color,t);
    float band=floor(t*5.0)/5.0;
    col*=0.93+0.07*smoothstep(band,band+0.03,t);
    if(cloud_amount>0.0){
        vec2 cu=vec2(atan(dir.z,dir.x)/6.28318+0.5, dir.y)*vec2(3.0,2.0);
        float cn=noise2c(cu*4.0)*0.5+noise2c(cu*8.0)*0.25+noise2c(cu*16.0)*0.125;
        float cloud=smoothstep(1.0-cloud_amount,1.0,cn)*clamp(dir.y*4.0,0.0,1.0);
        col=mix(col,cloud_color,cloud*0.85);
    }
    if(LIGHT0_ENABLED){
        ...
        if(sa<sr*1.3){
            float h=smoothstep(sr*1.3,sr*0.7,sa);
            col=mix(col,sun_disk_color*sun_energy*LIGHT0_ENERGY,h);
        }
    }
...
uniform float sun_energy : hint_range(0.0,8.0) = 1.0;
uniform float sky_exposure : hint_range(0.0,4.0) = 1.0;
uniform float star_density : hint_range(0.0,100.0) = 20.0;
...
float noise2a(vec2 p){ vec2 i=floor(p); vec2 f=fract(p); f=f*f*(3.0-2.0*f);
...
void sky(){
    ...
    col=mix(sky_horizon_color,sky_top_color,t1);
    float banded=floor(alt*6.0)/6.0;
    col*=0.94+0.06*smoothstep(0.0,0.08,fract(alt*6.0));
    if(LIGHT0_ENABLED){
        float cos_t=dot(dir,LIGHT0_DIRECTION);
        float se=LIGHT0_DIRECTION.y;
        float sf=clamp((0.4-abs(se))*3.0,0.0,1.0);
        float ns=pow(max(0.0,cos_t),3.0)*pow(max(0.0,1.0-alt*4.0),2.0);
        col=mix(col,sunset_color,ns*sf*sunset_strength*0.7);
        float sa=acos(clamp(cos_t,-1.0,1.0));
        float sr=0.025*sun_disk_scale;
        if(sa<sr*1.5){
            float h=smoothstep(sr*1.5,sr*0.6,sa);
            col=mix(col,sun_disk_color*sun_energy*LIGHT0_ENERGY,h*0.9);
        }
        float sv=clamp(0.3-se*3.0,0.0,1.0);
        if(star_density>0.0&&sv>0.0){
    ...
    if(cloud_softness>0.0&&alt>0.02){
        vec2 cu=vec2(atan(dir.z,dir.x)/6.28318+0.5,dir.y)*vec2(2.5,3.0);
        float cn=noise2a(cu*3.0)*0.5+noise2a(cu*6.0)*0.3+noise2a(cu*12.0)*0.2;
        float cloud=smoothstep(1.0-cloud_softness*0.4,1.0,cn)*clamp(alt*5.0,0.0,1.0);
        col=mix(col,cloud_color,cloud*0.6);
    }
    COLOR=col*sky_exposure;
}
)"; }
...
uniform float star_density : hint_range(0.0,100.0) = 60.0;
...
uniform float milky_way_strength : hint_range(0.0,2.0) = 0.5;
uniform float sky_exposure : hint_range(0.0,4.0) = 1.0;
float hash21n(vec2 p){ return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453); }
float noise2n(vec2 p){ vec2 i=floor(p); vec2 f=fract(p); f=f*f*(3.0-2.0*f);
...
float milky_way_band(vec3 dir){
    ...
    float mw=noise2n(vec2(b*4.0,dir.y*8.0))*0.5+noise2n(vec2(b*8.0,dir.y*16.0))*0.25;
    mw+=noise2n(vec2(b*16.0,dir.y*32.0))*0.125;
...
void sky(){
    ...
    if(LIGHT0_ENABLED){
        float cos_t=dot(dir,LIGHT0_DIRECTION);
        float ma=acos(clamp(cos_t,-1.0,1.0));
        float mr=0.015*moon_size;
        if(ma<mr*1.4){
            float h=smoothstep(mr*1.4,mr*0.8,ma);
            float tt=ma/max(mr,0.0001);
            float limb=mix(0.55,1.0,sqrt(max(0.0,1.0-tt*tt)));
            col=mix(col,moon_color*limb*moon_energy,h);
        }
        float mg=exp(-ma*ma/0.05);
        col+=moon_color*mg*moon_energy*0.04;
    }
    col*=sky_exposure;
    COLOR=max(col,vec3(0.0));
}
...
uniform float sky_exposure : hint_range(0.0,8.0) = 1.0;
uniform float star_density : hint_range(0.0,100.0) = 20.0;
uniform float star_brightness : hint_range(0.0,2.0) = 0.5;
...
void sky(){
    ...
    vec3 col=mix(sky_horizon_color,sky_mid_color,t1);
    col=mix(col,zenith_color,t2);
    float horiz=pow(max(0.0,1.0-alt*8.0),4.0);
    col=mix(col,horizon_fog_color,horiz*horizon_fog_amount);
    float sun_elev=LIGHT0_DIRECTION.y;
    if(LIGHT0_ENABLED){
        float cos_t=dot(dir,LIGHT0_DIRECTION);
        float glow=pow(max(0.0,cos_t),5.0)*clamp(1.0-alt*3.0,0.0,1.0);
        col=mix(col,sun_glow_color,glow*0.6);
        float sa=acos(clamp(cos_t,-1.0,1.0));
        float sr=0.02*sun_disk_scale;
        float cr=sr*(1.0+sun_corona_size*2.0);
        float corona=exp(-sa*sa/max(cr*cr,0.0001));
        col+=sun_disk_color*LIGHT0_ENERGY*corona*sun_energy*0.6;
        if(sa<sr){
            float tt=sa/max(sr,0.0001);
            float limb=mix(0.4,1.0,sqrt(max(0.0,1.0-tt*tt)));
            col+=sun_disk_color*LIGHT0_ENERGY*limb*sun_energy*4.0;
        }
    }
    float sv=clamp(alt-0.3,0.0,0.7)*clamp(0.5-sun_elev*2.0,0.0,1.0);
    if(star_density>0.0&&sv>0.0){
        vec2 uv=vec2(atan(dir.z,dir.x)/(2.0*PI)+0.5,
    ...
    void _update_shader_uniforms() {
        ...
        _smat->set_shader_parameter("cloud_amount",       0.3f);
        _smat->set_shader_parameter("cloud_softness",     0.5f);
    }
...
class ColorCorrectionEffect : public Node {
    ...
    void _apply() {
        ...
        if (tint_amount > 0.001f) {
            // CORRECCIÓN: Separamos el Math::max en dos llamadas porque Godot solo acepta de 2 en 2
...
class DepthOfFieldEffect : public Node {
    ...
    void _apply() {
        ...
        Ref<CameraAttributesPractical> cam;
        cam.instantiate();
        cam->set_dof_blur_far_enabled(enabled && far_enabled);
        cam->set_dof_blur_far_distance(far_distance);
        cam->set_dof_blur_far_transition(far_transition);
        cam->set_dof_blur_near_enabled(enabled && near_enabled);
